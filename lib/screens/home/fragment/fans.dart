...
class _FansState extends State<Fans> {
    ...
    FacebookInterstitialAd.loadInterstitialAd(
      ...
      listener: (result, value) {
        ...
        if (result == InterstitialAdResult.DISMISSED) {
          if (status_route != null) Navigator.push(context, status_route);
  ...
  addDownload(Status status) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (prefs.containsKey('downloaded_status_' + status.id.toString()) !=
        true) {
      ...
      try {
        response = await http
            .post(apiRest.addStatusDownload(), body: {'id': id_base_64});
        jsonData = convert.jsonDecode(response.body);
      } catch (ex) {
        print(ex);
        statusCode = 500;
  ...
  navigate(Status status, Function liked, Function shared, Function viewed,
      Function downloaded) {
    if (status.kind == "quote") {
      status_route = MaterialPageRoute(
          builder: (context) => QuoteDetail(
              status: status, liked: liked, shared: shared, viewed: viewed));
