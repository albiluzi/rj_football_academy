...
class _SondageWidgetState extends State<SondageWidget> {
  ...
  setAnswer(index) {
    setState(() {
      if (widget.question.multi) {
        ...
        answers.clear();
        answers.add(index);
        widget.question.answers[index].votes =
            (widget.question.answers[index].votes + 1);
      }
    });
  }
      ...
      for (Answer answer in widget.question.answers) {
        if (i == IndexCurrent) {
          answers_string = answers_string +
              "_" +
              widget.question.answers[IndexCurrent].id.toString();
        }
        IndexCurrent++;
      }
    }
  ...
  Future<String> _submitAnswer(String answers_string) async {
    ...
    try {
      response = await http.post(apiRest.submitAnswer(), body: {
        'question': widget.question.id.toString(),
                    ...
                    children: [
                      Container(
                          ...
                          width: (MediaQuery.of(context).size.width - 40) *
                              ((widget.question.answers[index].votes == 0 ||
                                      widget.question.getVotes() == 0)
                                  ? 0
                                  : (widget.question.answers[index].votes /
                                      widget.question.getVotes())),
                          decoration: BoxDecoration(
                            color:
                                Theme.of(context).primaryColor.withOpacity(0.2),
