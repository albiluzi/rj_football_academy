...
class _CommentsListState extends ResumableState<CommentsList> {
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: Stack(
            children: [
              Positioned(
                  bottom: (widget.post != null)
                      ? ((widget.post.comment == true) ? 60 : 0)
                      : ((widget.status.comment == true) ? 60 : 0),
                  left: 0,
                  right: 0,
                  top: 0,
                  child: Container(
  ...
  Future<List<Comment>> _getList() async {
    ...
    try {
      response =
          await http.get(apiRest.getCommentsBy(widget.post, widget.status));
  ...
  Future<String> _submitComment() async {
    ...
    if (logged) {
      ...
      try {
        response = await http
            .post(apiRest.submitComment(widget.post, widget.status), body: {
          "key": key_user,
      ...
      if (statusCode == 200) {
        if (jsonData["code"] == 200) {
          ...
          setState(() {
            ...
            if (widget.status != null) widget.status.comments += 1;

            if (widget.post != null) widget.post.comments += 1;
